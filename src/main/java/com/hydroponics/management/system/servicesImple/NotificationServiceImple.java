...
public class NotificationServiceImple implements NotificationServices {
	...
	public Notification sendNotificationAfterVerify(Notification notification, double hour) {
		...
		List<Notification> recentNotifications = notificationRepository
				.findByReceiverAndNotificationTypeAndEnvironmentAndTimestampAfter(notification.getReceiver(),
						notification.getNotificationType(), notification.getEnvironment(), tenHoursAgo);
	...
	@Override
	public PageableResponse getMyNotifications(User user, int pageNumber, int pageSize, String sortBy,
	...
	public void checkAndNotifyError(double actualValue, double baseValue, String fieldName,
		...
		if (!helperServices.isValidFieldData(actualValue, baseValue, Constants.MINERAL_ALLOWENCE_PERCENT)) {
			String errorMsg = "Hello Shubrato, Error in " + fieldName + ". Actual value is: " + actualValue
					...
					+ helperServices.givenPercentDecrease(baseValue, Constants.MINERAL_ALLOWENCE_PERCENT) + " to "
					+ helperServices.givenPercentIncrease(baseValue, Constants.MINERAL_ALLOWENCE_PERCENT) + ".";

			String smsMsg = "Hello " + environment.getOwnedBy().getFirstName() + ", there's an issue with your "
					+ environment.getPlantName() + " plants environment. " + fieldName.toUpperCase() + " is "
					+ String.format("%.3f", actualValue) + ", should be "
					+ helperServices.givenPercentDecrease(baseValue, Constants.MINERAL_ALLOWENCE_PERCENT) + " to "
					+ helperServices.givenPercentIncrease(baseValue, Constants.MINERAL_ALLOWENCE_PERCENT)
			...
			Notification sendNotification = this.sendNotificationAfterVerify(notification,
			...
			if (sendNotification != null) {
				...
				simpMessagingTemplate.convertAndSend("/specific/notification/" + sendNotification.getReceiver().getId(),
						sendNotification);
			} else {
	...
	public void checkMineralAndNotifyError(double actualValue, double baseValue, String fieldName,
		...
		if (!helperServices.isValidFieldData(actualValue, baseValue, Constants.MINERAL_ALLOWENCE_PERCENT)) {
			String errorMsg = "Error in " + fieldName + ". Actual value is: " + String.format("%.2f", actualValue)
					+ ". It should be within the range of "
					+ helperServices.givenPercentDecrease(baseValue, Constants.MINERAL_ALLOWENCE_PERCENT) + " to "
					+ helperServices.givenPercentIncrease(baseValue, Constants.MINERAL_ALLOWENCE_PERCENT) + ".";
			log.error(errorMsg);
			...
			String smsMsg = "Hello " + environment.getOwnedBy().getFirstName() + ", there's an issue with your "
					+ environment.getPlantName() + " plants environment. " + fieldName.toUpperCase() + " is "
					+ String.format("%.2f", actualValue) + ", should be "
					+ helperServices.givenPercentDecrease(baseValue, Constants.MINERAL_ALLOWENCE_PERCENT) + " to "
					+ helperServices.givenPercentIncrease(baseValue, Constants.MINERAL_ALLOWENCE_PERCENT)
					+ ". Please address this. Thank you.";
			...
			Timestamp now = new Timestamp(System.currentTimeMillis());
			Timestamp tenHoursAgo = new Timestamp(
					now.getTime() - ((long) Constants.NOTIFICATION_TIME_INTERVAL_HOUR * 60 * 60 * 1000));

			List<Notification> recentNotifications = notificationRepository
					.findByReceiverAndNotificationTypeAndEnvironmentAndMineralAndTimestampAfter(
							notification.getReceiver(), notification.getNotificationType(),
							notification.getEnvironment(), notification.getMineral(), tenHoursAgo);
	...
	public Notification sendEnvWelcomeNotification(Environment addEnvironment) {
		...
		String msg = "🌱 Welcome to the Green Family, " + addEnvironment.getOwnedBy().getFirstName() + "!\n\n"
				+ "We're excited to share the news of your brand-new hydroponics environment. Your oasis, located in "
				+ addEnvironment.getLocation().getFullAddress() + ", is now home to thriving "
				+ addEnvironment.getPlantName() + ". 🌿\n\n" + "A big shoutout to "
				+ addEnvironment.getAddedEnvironmentBy().getFirstName()
				+ " for making it happen! 🌟 Get ready for a journey of growth, freshness, and green goodness.\n\n"
	...
	public Notification sendPaymentNotification(Payment payment) {
		...
		case PAID:
			...
			notification.setMessage("Thank you! Your payment of " + payment.getAmount() + " taka for Invoice INV-000"
					+ payment.getInvoice().getId() + " has been received successfully.");
			break;
		case PARTIAL_PAID:
			notification.setNotificationType(NotificationType.SUCCESS_PARTIAL_PAYMENT);
			notification.setMessage(
					"Thank you! Your partial payment of " + payment.getAmount() + " taka for Invoice INV-000"
							+ payment.getInvoice().getId() + " has been received successfully. Remaining amount: "
		...
		default:
			...
			notification.setMessage("Reminder: Your payment of " + payment.getAmount() + " taka for Invoice INV-00"
		...
		Notification save = notificationRepository.save(notification);
		smsServices.sendSms(save.getReceiver().getPhone(), "Hello " + save.getReceiver().getFirstName() + ", "
				+ save.getMessage() + " Best regards, LeafLab Team");
		return save;
	}
	...
	public List<Notification> getUnreadNotifications(User user) {
		List<Notification> findByReceiverAndStatus = notificationRepository.findByReceiverAndStatus(user,
